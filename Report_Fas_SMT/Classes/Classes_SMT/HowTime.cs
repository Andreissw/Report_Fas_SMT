...
namespace Report_Fas_SMT.Classes
{
    ...
    {       
        ...
        void GetShift()
        {
            //var NOW = DateTime.UtcNow.AddHours(11).AddDays(-4).Hour;
            var datenow = DateTime.UtcNow.AddHours(-5);
            //var datenow = DateTime.UtcNow.AddHours(11).AddDays(-4);
            var NOW = datenow.Hour;
            if (NOW >= 8 & NOW <= 20)
            {
                TimeList = ListTimesDay;
                mode.DateSt = DateTime.Parse(datenow.ToString("yyyy-MM-dd") + " 08:00:00");
                mode.Dateend = DateTime.Parse(datenow.ToString("yyyy-MM-dd") + " 20:00:00");     
                TypeDay = "Day";
            ...
            else
            {
                if (NOW >= 20 & NOW <= 23)
                {                   
                        mode.DateSt = DateTime.Parse(datenow.AddDays(0).ToString("yyyy-MM-dd") + " 20:00:00");
                        mode.Dateend = DateTime.Parse(datenow.AddDays(1).ToString("yyyy-MM-dd") + " 08:00:00");                        
                }
                else
                {                    
                        mode.DateSt = DateTime.Parse(datenow.AddDays(-1).ToString("yyyy-MM-dd") + " 20:00:00");
                        mode.Dateend = DateTime.Parse(datenow.AddDays(0).ToString("yyyy-MM-dd") + " 08:00:00");                                     
                }
        ...
        void TypeDayGet()
        {            
            var NOW = DateTime.UtcNow.AddHours(-5).Hour;
