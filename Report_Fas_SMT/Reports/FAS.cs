...
namespace Report_Fas_SMT.Reports
{
    public class FAS
    {
        ...
        CounterStage GetInfoOrder(DataSet data,string fullLotcode)
        {
        ...
        public bool GetDataFAS() {
            ...
            string fulllotcode = fas.Contract_LOT.Where(c => c.ID == idlot).FirstOrDefault().FullLOTCode;


            DataFas.CounterStages.Add(GetInfoOrder(result, fulllotcode));





        DataFas.DataContract.ListLog = fas.Ct_OperLog.Where(c => c.StepDate >= now).GroupBy(c=> new { c.StepID,c.TestResultID, c.LOTID }).Select(c => new LogContract { 
            ...
            DataFas.DataContract.ListTop = fas.Ct_OperLog.Where(c => c.StepDate >= now & c.TestResultID == 3 & c.ErrorCodeID != null & c.ErrorCodeID != 580).GroupBy(c => new { c.LOTID, c.ErrorCodeID }).Select(c => new TopErrorList_ct
            {
                Count = c.Count(),
            ...
            DataFas.DataContract.ListFPY = fas.Ct_OperLog.Where(c => c.StepDate >= now & c.LOTID != null & c.ErrorCodeID != 580).GroupBy(c=> new { c.LOTID }).Select(c => new FPY_Ct
            {
            ...
            var Result = fas.M_Repair_Table.Where(c => c.RapairDate >= now & c.Position != null).Select(c => new RepairTable
            {
                Barcode = c.Barcode,
                DefectCode = fas.FAS_DefectCode.Where(b=>b.NameCode == c.DefectCode).Select(b=>b.NameCode + " " + b.DescriptionCode).FirstOrDefault(),
                RepairName = fas.FAS_RepairCode.Where(b=>b.NameCode == c.RepairCode).Select(b=>b.NameCode + " " + b.DescriptionCode).FirstOrDefault(),
            ...
            foreach (var item in Result)
            {
                ...
                var pcbid = smd.LazerBase.Where(c => c.Content == item.Barcode).Select(c=>c.IDLaser).FirstOrDefault();               

                if (pcbid != 0)
                {
                    var LOTID = fas.Ct_OperLog.Where(c => c.PCBID == pcbid & c.LOTID != null).Select(c=>c.LOTID).FirstOrDefault();
                    if (LOTID != null)
                    {                   
                        if (LOTID != 0)
                        {
                            item.FullLotCode = fas.Contract_LOT.Where(c => c.ID == LOTID).Select(c=>c.FullLOTCode).FirstOrDefault();
                            continue;
                        }
                    }

                    var serialNumber = fas.FAS_OperationLog.Where(c => c.PCBID == pcbid & c.SerialNumber != null).Select(c=>c.SerialNumber).FirstOrDefault();
                    if (serialNumber != null)
                    {
                        if (serialNumber != 0)
                        {
                            LOTID = fas.FAS_SerialNumbers.Where(c => c.SerialNumber == serialNumber).Select(c=>c.LOTID).FirstOrDefault();
                            item.FullLotCode = fas.FAS_GS_LOTs.Where(c => c.LOTID == LOTID).Select(c=>c.FULL_LOT_Code).FirstOrDefault();
                            continue;

                        }
                    }

                    foreach (var model in new List<string>() { "BTH", "BTA" }) {
                        if (item.Barcode.Length > 11)   
                            if (item.Barcode.Substring(7, 3) == model)
                            {
                                item.FullLotCode = "Barton " + model; break;
                            }
                ...
                var SNID = fas.Ct_FASSN_reg.Where(c => c.SN == item.Barcode).Select(c=>c.ID).FirstOrDefault();
                var LOTIDs = fas.Ct_OperLog.Where(c => c.SNID == SNID & c.LOTID != null).Select(c=>c.LOTID).FirstOrDefault();
                if (LOTIDs != null)            
                    if (LOTIDs != 0) {                 
                        item.FullLotCode = fas.Contract_LOT.Where(c => c.ID == LOTIDs).Select(c=>c.FullLOTCode).FirstOrDefault();
                        continue;
                    }
            ...
            DataFas.DataOther.ListRepair = Result.Where(c=>c.FullLotCode != "").GroupBy(c => new { c.RepairName, c.PositionName, c.FullLotCode }).Select(c => new ReapirFAS()
            {
                PositionName = c.Key.PositionName,
                //DefectCode = c.Key.DefectCode,
                ModelName = c.Key.FullLotCode,
                Count = c.Count(),
                RepairCode = c.Key.RepairName,                 

            }).OrderBy(c=>c.ModelName).ThenByDescending(c=>c.Count).ToList();
...
<td>
                                  <table class = ""intd""><!-- Ремонт общий -->
                                   <th class = ""HeadTable"" colspan=""5""> Ремонт FAS</th>  
                                    {Repairs}
                                  </table>
  </td>
<td>
