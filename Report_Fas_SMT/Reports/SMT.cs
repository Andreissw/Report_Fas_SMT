...
namespace Report_Fas_SMT.Classes
{
    ...
    public  class SMT
    {
        ...
        double? GetCell(string model)
        {
            ...
            var LOTID = fas.EP_PGName.Include(c => c.EP_Protocols).Where(c => c.Name == model).Select(c => c.EP_Protocols.LOTID).FirstOrDefault();

            if (LOTID == 0) return 0;

            return fas.FAS_Objective.Where(c => c.LOTID == LOTID & c.Manuf == "Цех Поверхностного монтажа").Select(c => c.Objective).FirstOrDefault();
